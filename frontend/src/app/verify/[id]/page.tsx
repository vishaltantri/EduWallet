...
export default function VerifyPage() {
  ...
  function runVerificationAnimation(result: VerificationData) {
    setData(result);
    setCurrentStep(0);
    setShowResult(false);
    let step = 0;
  ...
  async function fetchVerification(signal: AbortSignal): Promise<(() => void) | undefined> {
    try {
      ...
      if (!signal.aborted) return runVerificationAnimation(result);
    } catch (err: unknown) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setQrUrl(window.location.href), 0);
  ...
  useEffect(() => {
    ...
    const startTimer = window.setTimeout(() => {
      ...
      if (id === "demo") {
        ...
        void fetchVerification(controller.signal).then((stop) => {
          if (cancelled) stop?.();
          else stopAnimation = stop;
        });
      }
    }, 0);
